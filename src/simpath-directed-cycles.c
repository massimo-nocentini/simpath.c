...
void printstate(int j, int jj, int i, int k)
{
    ...
    {
        t = f[p];
        if (nextfront[t] != i + 1 && mate[t] && mate[t] != t)
            break;
    }
    if (p < s)
        printf("0");
    else if (nextfront[j] != i + 1 && mate[j] && mate[j] != j)
        printf("0");
    else if (nextfront[k] != i + 1 && mate[k] && mate[k] != k)
        printf("0");
    else if (ss == 0)
        printf("%x",
               newserial);
...
#line 259 "simpath-directed-cycles.w"
        ...
        if (j == jj && nextfront[j] != i + 1)
            nextfront[j] = i + 1, ff[ss++] = j;
        if (k >= jj && nextfront[k] != i + 1)
            nextfront[k] = i + 1, ff[ss++] = k;
